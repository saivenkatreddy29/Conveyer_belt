...
# Initializes the ZED camera, processes frames, and calculates belt speed using either feature extraction or optical flow.
def process_video(video_path, log_file_raw,log_file_smooth,window_size=5,fps=60,method = 'feature_extraction'):
    """
   Process a ZED camera video file to calculate belt speed using either feature extraction or optical flow methods.

   Args:
       video_path (str): Path to the SVO video file from ZED camera
   ...
   Notes:
       ...
       - Uses fixed ROI coordinates (510, 250, 190, 150)
   ...
   # Seting runtime parameters 
    ...
    prev_depth_map = None 
    frame_no = 0
    path_to_speed = log_file_raw+method+'.csv'
    ...
    while key != 113:
        ...
        if method == 'feature_extraction':
            speed,curr_kp,curr_des = implement_orb(frame, prev_kp, prev_depth_map, curr_depth_map, camera_parameters, prev_des, roi,fps)
            prev_kp, prev_des = curr_kp, curr_des
        elif method == 'optical_flow':
            curr_gray = cv2.cvtColor(frame[y:y+h, x:x+w], cv2.COLOR_BGR2GRAY)
            speed = implement_optical_flow(prev_gray,curr_gray,prev_depth_map,curr_depth_map,camera_parameters,x,y,fps)
            prev_gray = curr_gray
...
if __name__ == "__main__":
    video_path = "/home/pawankalyan2905/belt_sample.svo"
    log_file_raw = "belt_speed_"
    ...
    parser.add_argument("--method", type=str, choices=["feature_extraction", "optical_flow"], default="feature_extraction", help="Method for processing.")
    args = parser.parse_args()
    process_video(video_path, log_file_raw,log_file_smooth,window_size= args.window_size,fps=60,method = args.method)
...
In the plot, we can see that optical flow has a smoother curve than feature detection even without using dedicated outlier detection techniques. This is because optical flow computes motion vectors for all pixels (or a dense subset, e.g., every 20 pixels), providing a continuous representation of motion. In contrast, feature extraction relies on sparse keypoints, which can lead to noisier estimates if keypoints are unevenly distributed or mismatched. This doesn't mean that it is always recommended to use optical flow because of its computational cost. As we increase the frequency of comparisons, the program requires more computational resources. So, considering this tradeoff, the choice of method should be done carefully.
